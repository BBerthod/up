...
const submit = () => form.post(route('status-pages.store'))
</script>
...
<template>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Create Status Page</h1>
            <p class="text-slate-400 mt-1">Set up a public status page for your users.</p>
        </div>

        <form @submit.prevent="submit" class="glass p-6 space-y-6">
            <div>
                <label class="block text-sm font-medium text-white mb-2">Name</label>
                <input v-model="form.name" type="text" class="form-input w-full" placeholder="My Service Status" required />
            ...
            <div>
                ...
                <textarea v-model="form.description" rows="2" class="form-input w-full" placeholder="Current status of our services" />
            </div>
            ...
            <div>
                ...
                <p v-else class="text-slate-500 text-sm py-4 text-center bg-white/5 rounded-lg">No monitors available</p>
            ...
            <button type="submit" :disabled="form.processing" class="w-full py-3 px-4 rounded-lg text-white font-semibold bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-600 hover:to-teal-600 transition-all shadow-lg shadow-cyan-500/20 disabled:opacity-50">
                {{ form.processing ? 'Creating...' : 'Create Status Page' }}
            </button>
        </form>
    </div>
</template>
